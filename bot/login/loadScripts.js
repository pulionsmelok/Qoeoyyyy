...
async function loadCommands(api) {
    const unloadList = (global.GoatBot.configCommands?.commandUnload || []).map(n => String(n).toLowerCase());
    const allowList = (global.GoatBot.configCommands?.commandAllowLoad || []).map(n => String(n).toLowerCase());
    ...
    for (const file of files) {
        ...
        const bare = lower.replace(".js", "");
        if (unloadList.includes(lower) || unloadList.includes(bare)) {
            spinner.update(`Skipping command: ${file}`);
            skipped++;
            continue;
        }
        if (allowList.length > 0 && !allowList.includes(lower) && !allowList.includes(bare)) {
            skipped++;
            continue;
        }
