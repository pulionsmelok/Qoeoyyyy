...
module.exports = {
  config: {
    ...
    longDescription:
      "A mini-game where the bot sends a flag and you have to guess the country to win Money.",
    category: "game",
    guide: {
  ...
  onReply: async function ({
    ...
    if (isCorrect) {
      ...
      return message.reply(
        `❌ | Wrong Answer! (${newAttempts}/${maxAttempts})`,
      );
    }
  },
  ...
  onStart: async function ({
    ...
    try {
      ...
      if (args[0] === "list" || args[0] === "top") {
        ...
        if (sorted.length === 0)
          return message.reply("🏆 | No wins recorded yet.");
          ...
          if (u && u.name && u.name !== "Unknown") dName = u.name;
            ...
            try {
              ...
              if (sock && sock.contacts && sock.contacts[uid]) {
                dName =
                  sock.contacts[uid].notify || sock.contacts[uid].name || uid;
              }
            } catch (e) {}
          }
      ...
      if (args[0] === "reset") {
        const isBotAdmin =
          global.GoatBot.config.adminIDs &&
          global.GoatBot.config.adminIDs.includes(event.senderID);
        ...
        if (!isBotAdmin && !isAdmin)
          return message.reply(
            "⚠️ | Permission denied. Only Admins can reset.",
          );
