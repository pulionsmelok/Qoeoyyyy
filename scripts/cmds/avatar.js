...
module.exports = {
  config: {
    ...
    longDescription:
      "Set the bot's profile picture by replying to an image or providing an image URL.",
    category: "admin",
      ...
      if (!buffer) {
        if (dlError)
          return message.reply(
            "❌ Failed to download replied image: " + dlError,
          );
        return message.reply(
          "❌ Please reply to an image or provide a valid image URL to set the avatar.",
        );
      }
