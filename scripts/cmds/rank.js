...
let deltaNext;
const expToLevel = (exp, deltaNextLevel = deltaNext) =>
  Math.floor((1 + Math.sqrt(1 + (8 * exp) / deltaNextLevel)) / 2);
const levelToExp = (level, deltaNextLevel = deltaNext) =>
  ...
  onStart: async function ({
    ...
    const rankCards = await Promise.all(
      targetUsers.map(async (userID) => {
        const rankCard = await makeRankCard(
          userID,
          userData,
          threadsData,
          event.threadID,
...
async function makeRankCard(
  ...
  const expNextLevel =
    levelToExp(levelUser + 1, deltaNext) - levelToExp(levelUser, deltaNext);
  const currentExp =
    expNextLevel - (levelToExp(levelUser + 1, deltaNext) - exp);

  const allUser = await userData.getAll();
  allUser.sort((a, b) => (b.exp || 0) - (a.exp || 0));
  const norm =
    global.normUID || ((id) => String(id).split("@")[0].split(":")[0]);
  const targetID = norm(userID);
  const rank = allUser.findIndex((user) => norm(user.userID) === targetID) + 1;

  const customRankCard = await threadsData.get(
    threadID,
    "data.customRankCard",
    {},
  );
  ...
  if (global.utils) {
    ...
    if (
      global.utils.getLiveMemberName &&
      (displayName === "Unknown" || displayName === userID)
    ) {
...
class RankCard {
  ...
  setExpBarColor(expNextLevel_color) {
    if (
      typeof expNextLevel_color !== "string" &&
      !Array.isArray(expNextLevel_color)
    )
  ...
  async buildCard() {
    ...
    await checkColorOrImageAndDraw(
      alignRim,
      alignRim,
      widthCard - alignRim * 2,
      heightCard - alignRim * 2,
      ctx,
      sub_color,
      20,
      alpha_subcard,
    ...
    if (line_color) {
      if (!isUrl(line_color)) {
        ...
        ctx.arc(
          xyAvatar,
          xyAvatar,
          resizeAvatar / 2 + heightLineBetween,
          0,
          2 * Math.PI,
        );
        ctx.fill();
        ...
        ctx.rect(
          xyAvatar + resizeAvatar / 2,
          heightCard / 2 - heightLineBetween / 2,
          widthLineBetween,
          heightLineBetween,
        );
        ctx.fill();
    ...
    if (!isUrl(expNextLevel_color)) {
      ...
      ctx.drawImage(
        await Canvas.loadImage(expNextLevel_color),
        xStartExp,
        yStartExp,
        widthExp + radius,
        heightExp,
    ...
    if (!isUrl(exp_color)) {
      ...
      ctx.lineTo(
        xStartExp + widthExpCurrent + radius,
        yStartExp + heightExp - radius,
      );
      ctx.arcTo(
        xStartExp + widthExpCurrent + radius,
    ...
    // Draw rank text
    ctx.font = autoSizeFont(
      18.4 * percentage(widthCard),
      maxSizeFont_Rank,
      rank,
      ctx,
      this.fontName,
    ...
    ctx.fillText(
      name,
      47.5 * percentage(widthCard),
      40 * percentage(heightCard),
    );

    // Draw Exp text
    const textExp = `Exp ${exp}/${expNextLevel}`;
    ctx.font = autoSizeFont(
      49 * percentage(widthCard),
      maxSizeFont_Exp,
      textExp,
      ctx,
      this.fontName,
    ...
    if (main_color.match?.(/^https?:\/\//) || Buffer.isBuffer(main_color)) {
      ...
      ctx.clip();
      ctx.drawImage(
        await Canvas.loadImage(main_color),
        0,
        0,
        widthCard,
        heightCard,
...
async function checkColorOrImageAndDraw(
  xStart,
  yStart,
  width,
  height,
...
function drawSquareRounded(
  ...
  r,
  color,
  defaultGlobalCompositeOperation,
  notChangeColor,
) {
  ctx.save();
...
function checkFormatColor(color, enableUrl = true) {
  if (
    ...
    throw new Error(
      `The color format must be a hex, rgb, rgba ${enableUrl ? ", url image" : ""} or an array of colors`,
    );
}
