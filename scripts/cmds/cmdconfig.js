module.exports = {
  config: {
    ...
    longDescription:
      "Shows current command/event skip lists from configCommands.json.",
    category: "admin",
    guide: { en: "{pn}" },
  },
  ...
  onStart: async ({ api, event, message }) => {
    ...
    text +=
      (cc.commandUnload || []).length > 0
        ? cc.commandUnload.join(", ")
        : "(none)";
    text += "\n\n";
    text += `⚡ Event Unload (${(cc.commandEventUnload || []).length}):\n`;
    text +=
      (cc.commandEventUnload || []).length > 0
        ? cc.commandEventUnload.join(", ")
        : "(none)";
