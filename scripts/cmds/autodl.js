...
async function getDownloadData(url) {
  ...
  if (!res.data?.success) {
    throw new Error(
      res.data?.message || res.data?.error || "Download API failed",
    );
  }
  return res.data;
}
...
module.exports = {
  config: {
    ...
    longDescription:
      "Downloads videos from Instagram, TikTok, YouTube Shorts, Facebook, and more.",
  ...
  onStart: async ({ api, message, event, args }) => {
    ...
    try {
      ...
      if (!/^https?:\/\//i.test(url))
        return message.reply(
          "❌ Please provide a valid URL starting with http/https.",
        );
      ...
      if (status?.messageID)
        await message.unsend(status.messageID).catch(() => {});
