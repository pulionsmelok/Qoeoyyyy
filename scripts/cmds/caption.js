...
module.exports = { config,
	...
	const mentionIds = Array.isArray(event.mentions)
		? event.mentions
		: Object.keys(event.mentions || {});
	if (mentionIds.length === 1) {
		const userId = mentionIds[0];
		let mentionName = "বন্ধু";
		if (event.mentions && !Array.isArray(event.mentions) && event.mentions[userId]) {
			mentionName = String(event.mentions[userId]).replace(/^@/, "");
		} else if (typeof userId === "string") {
			mentionName = userId.split("@")[0].split(":")[0] || "বন্ধু";
		}
		await api.sendMessage(
			`💌 ${mentionName}-এর জন্য:\n\n${randomCaption}`,
			threadID,
			{ replyToMessage: event.raw }
		);
		return;
	}
	await api.sendMessage(
