  ...
  for (const attachment of attachments) {
    try {
      ...
      if (type === "photo" || type === "image") {
        ...
        await api.sendAudio(buffer, toJID, {
          ...msgOptions,
          ptt: type === "ptt" || attachment.ptt,
          mimetype,
        });
      } else if (type === "sticker") {
        await api.sendSticker(buffer, toJID, msgOptions);
      } else {
        await api.sendDocument(buffer, toJID, attachment.fileName || "file", {
          ...msgOptions,
          mimetype,
        });
      }
...
module.exports = {
  ...
  onStart: async function ({
    ...
    const msg =
      ...
      `\n- User ID: @${senderNum}` +
      (isGroup
        ? getLang("sendByGroup", threadName, threadID)
        : getLang("sendByUser"));
    const formMessage = {
    ...
    const adminNames = await Promise.all(
      resolvedAdminBot.map(async (item) => ({
        id: item,
    ...
    for (const uid of resolvedAdminBot) {
      ...
      try {
        ...
        const sentID =
          messageSend?.key?.id ||
          messageSend?.id ||
          (Array.isArray(messageSend) && messageSend[0]?.key?.id) ||
          null;
        if (sentID) {
          successIDs.push(uid);
    ...
    switch (type) {
      case "userCallAdmin": {
        ...
        try {
          ...
          const infoID =
            info?.key?.id ||
            info?.id ||
            (Array.isArray(info) && info[0]?.key?.id) ||
            null;
          if (infoID) {
          ...
          await message.reply(
            "❌ Failed to send reply to user: " + err.message,
          );
        }
        break;
      }
      ...
      case "adminReply": {
        ...
        try {
          ...
          await message.reply(
            "❌ Failed to send feedback to admin: " + err.message,
          );
        }
        break;
      }
