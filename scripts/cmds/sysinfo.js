...
async function collectInfo(api) {
  ...
  return {
    ...
    dbType:
      (global.GoatBot.config.database && global.GoatBot.config.database.type) ||
      "json",
    users,
    threads,
...
function drawFrame(ctx, info, frame) {
  ...
  drawBar(
    ctx,
    "Heap Usage",
    info.heapUsed / Math.max(info.heapTotal, 1),
    44,
    150,
    375,
    "#22d3ee",
    frame,
    `${formatBytes(info.heapUsed)} / ${formatBytes(info.heapTotal)}`,
  );
  drawBar(
    ctx,
    "System Memory",
    info.usedSystemMem / Math.max(info.totalMem, 1),
    480,
    150,
    375,
    "#22c55e",
    frame,
    `${formatBytes(info.usedSystemMem)} / ${formatBytes(info.totalMem)}`,
  );
  drawBar(
    ctx,
    "Process RSS",
    info.rss / Math.max(info.totalMem, 1),
    44,
    215,
    375,
    "#f59e0b",
    frame,
    formatBytes(info.rss),
  );
  drawBar(
    ctx,
    "Module Load",
    Math.min((info.commands + info.events) / 100, 1),
    480,
    215,
    375,
    "#a78bfa",
    frame,
    `${info.commands} cmds / ${info.events} events`,
  );
  ...
  drawText(
    ctx,
    `Network: Express ${info.express} | listenEvents ${info.listenEvents} | selfListen ${info.selfListen}`,
    44,
    488,
    15,
    "#8aa0b8",
  ...
  drawText(
    ctx,
    `Deps ${info.deps} | onReply ${info.onReply} | onReaction ${info.onReaction}`,
    690,
    488,
    15,
    "#8aa0b8",
...
module.exports = {
  ...
  onStart: async ({ api, message, event }) => {
    ...
    try {
      ...
      return await api.sendImage(
        buffer,
        event.threadID,
        `${info.botName} system info`,
        { mimetype: "image/png" },
      );
    } catch (e) {
