module.exports = {
  ...
  onStart: async ({ message, event, args, usersData, api }) => {
   ...
   if (winRoll <= 0.5) forceMatch = 4;
    ...
    await api.editMessageText({ chat_id: threadID, message_id: String(sent.messageID || sent.message_id), text: `🎰 | SLOT MACHINE\n──────────────\n [ ${s[0]} | ${s[1]} | ❓ | ❓ ]\n──────────────\n⌛ Spinning...` });
    await new Promise(r => setTimeout(r, 1000));
    const counts = {};
    ...
    await api.editMessageText({ chat_id: threadID, message_id: String(sent.messageID || sent.message_id), text: resultMessage });
  }
};
