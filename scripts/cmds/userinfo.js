...
module.exports = {
  config: {
    ...
    longDescription:
      "View profile info, message count, and profile picture of yourself or someone else (by tagging them or replying to their message).",
    category: "utility",
    ...
    try {
      ...
      if (event.isGroup) {
        try {
          ...
          const isAdmin =
            thread.adminIDs && thread.adminIDs.includes(targetJid);
      ...
      let buffer = null;
      try {
        const targetUrl = await global.db.usersData.getAvatarUrl(
          api,
          targetJid,
        );
