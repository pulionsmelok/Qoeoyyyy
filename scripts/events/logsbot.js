module.exports = {
  ...
  onStart: async ({ api, event, threadsData, userData }) => {
    ...
    const phone =
      (senderID || "").split(":")[0].split("@")[0] || senderID || "?";
    ...
    switch (logMessageType) {
      case "log:subscribe":
        global.log.info(
          "GROUP EVENT",
          `${c.greenBright("JOIN")} - ${groupName}: ${participantText}`,
        );
        break;
      case "log:unsubscribe":
        global.log.info(
          "GROUP EVENT",
          `${c.yellowBright("LEAVE")} - ${groupName}: ${participantText}`,
        );
        break;
      case "log:thread-admins":
        global.log.info(
          "GROUP EVENT",
          `${c.cyanBright("ADMIN")} - ${groupName}: ${action || "changed"} by ${phone}`,
        );
        break;
      case "log:thread-name":
        global.log.info(
          "GROUP EVENT",
          `${c.magentaBright("RENAME")} - ${groupName} (by ${phone})`,
        );
        break;
      ...
      case "log:thread-icon":
        global.log.info(
          "GROUP EVENT",
          `${c.blueBright("ICON")} - ${groupName} (by ${phone})`,
        );
        break;
      default:
        if (logMessageType) {
    ...
    if (event.isBotAdded)
      global.log.success(
        "BOT EVENT",
        `Bot was ADDED to: ${groupName} (${threadID})`,
      );
    if (event.isBotRemoved)
      global.log.warn(
        "BOT EVENT",
        `Bot was REMOVED from: ${groupName} (${threadID})`,
      );
    if (event.isBotPromoted)
      global.log.success(
        "BOT EVENT",
        `Bot was PROMOTED to admin in: ${groupName}`,
      );
    if (event.isBotDemoted)
      global.log.warn("BOT EVENT", `Bot was DEMOTED in: ${groupName}`);
  },
};
