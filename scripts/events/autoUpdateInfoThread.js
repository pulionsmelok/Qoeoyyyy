module.exports = {
  ...
  onStart: async ({ api, event, threadsData, userData }) => {
    ...
    const { threadID, logMessageType, logMessageData, action, participants } =
      event;
    if (!threadID) return;
    ...
    try {
      switch (logMessageType) {
        case "log:subscribe": {
          ...
          const added = Array.isArray(participants)
            ? participants
            : (logMessageData && logMessageData.addedParticipants) || [];
          const thread = await threadsData.get(threadID);
          const memberMap = {};
          for (const m of thread.allMembers || []) memberMap[m.uid] = m;
          for (const uid of added) {
            const name = await global.resolveUserDisplayName(
              api,
              uid,
              userData,
            );
            if (!memberMap[uid])
              ...
              if (name && name !== uid.split(":")[0].split("@")[0])
                memberMap[uid].name = name;
            }
          }
          await threadsData.set(threadID, {
            allMembers: Object.values(memberMap),
            totalMember: Object.values(memberMap).filter((m) => m.inGroup)
              .length,
          });
        ...
        case "log:unsubscribe": {
          ...
          const removed = Array.isArray(participants)
            ? participants
            : (logMessageData && logMessageData.removedParticipants) || [];
          const thread = await threadsData.get(threadID);
        ...
        case "log:thread-name": {
          const newName =
            (logMessageData && logMessageData.value) ||
